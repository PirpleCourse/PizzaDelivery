...
module.exports = (router) => {
  ...
  /**
   * Payments API route controller
   */

  // Verify a new payment made to an item
  router.addRoute('payment', 'post', (data, callback) => {

  }, helpers.verifyToken);

  // Get all payments made by a contact
  router.addRoute('payment', 'get', (data, callback) => {
  ...
  // Gets the current orders and details of the orders for the user
  // OR gets details for one order if explicitly passed an id as query param
  router.addRoute('order', 'get', (data, callback) => {
    var { id }
  }, helpers.verifyToken);
